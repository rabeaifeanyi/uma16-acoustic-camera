...
class MaskedSpectraOut(ac.SpectraOut):

    #: Data source; :class:`~acoular.base.Generator` or derived object.
    source = Instance(SpectraGenerator)

    indices = List(Tuple) # [(2, 3), (4, 5), (9, 15)]

    #: Number of frequencies in output, as given by :attr:`source`.
    numfreqs = Property(depends_on=['indices', 'source.numfreqs'])

    #: 1-D array of frequencies, as given by :attr:`source`.
    freqs = Property(depends_on=['indices', 'source.sample_freq', 'source.blocksize'])

    def _get_numfreqs(self):
        return # number of frequencies after masking

    def _get_freqs(self):
        return

    def get_freq_indices(self, f, num):
        # should return tuple with start and end index

    def result(self, num=1):
        # implement masking of frequencies
        pass
